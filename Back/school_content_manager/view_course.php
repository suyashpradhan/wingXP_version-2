...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $vid_up = "SELECT description_line,description, duration, learning, topic.topic_name,
    vendor.vendor_name, mrp_price, school_price,class_applicable_for,vendor.vendor_icon,activities.icon,primary_image,secondary_image,course_icon
     from school__live_course  INNER JOIN vendor ON 
    school__live_course.vendor_id =   vendor.vendor_id  INNER JOIN topic ON 
    school__live_course.topic_id =   topic.topic_id 
...
<body>
    ...
    <input type="hidden" name="id" id="id" value="<?php if(isset($id)){echo $id;}else{}?>">
           <input type="hidden" id="type" name="type" value="live_course">
