...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $art_up = "SELECT title,description,speaker,no_of_classes,class_applicable_for,
    learning,vendor.vendor_name,prerequisites,mrp_price,school_price,
    primary_image,secondary_image,course_icon,vendor.vendor_icon,activities.icon,
    start_time,end_time,duration,school__workshop.date,topic.topic_name
     from school__workshop 
     INNER JOIN vendor ON 
    school__workshop.vendor_id =   vendor.vendor_id INNER JOIN topic ON 
    school__workshop.topic_id =   topic.topic_id INNER JOIN activities ON
