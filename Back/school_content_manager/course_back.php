...
$description_line=mysqli_real_escape_string($conn,$_POST['title']);
...
{   
    ...
    {                            //File update
                            ...
                            $work_up = "SELECT primary_image,secondary_image,course_icon from school__live_course where course_id = '$course_id'; ";                            
                            $work_up .= "UPDATE  school__live_course SET description_line = '$description_line', description='$description',
                            school_price='$school_price',mrp_price='$price',duration='$duration',learning='$learning',
                            ...
                            $work_up .= "vendor_id='$vendor',club_id='$club_id',inst_id='$inst_id',cc_id='$Userid' where course_id= '$course_id'";
    ...
    {  
        ...
        $check="SELECT * FROM school__live_course WHERE description_line = '$description_line'";
        ...
        else 
        {
                             ...
                             $i=ren_save('icon');
                            //Data Upload                             
                            $sql = "INSERT INTO school__live_course  (description_line,description,mrp_price,school_price,duration,learning,class_applicable_for,subscription_level,topic_id,";
                            ...
                            if ($_FILES['icon']['name']==''){}else{ $sql .= "course_icon,";}
                             $sql .= "vendor_id,club_id,inst_id,cc_id) VALUES ('$description_line','$description','$price','$school_price','$duration','$learning','$class','$sub','$topic',";
                             if ($_FILES['primary']['name']==''){}else{ $sql .= "'$p',";}
                             if ($_FILES['secondary']['name']==''){}else{ $sql .= "'$s',";}
                             if ($_FILES['icon']['name']==''){}else{ $sql .= "'$i',";} 
                             $sql .= "'$vendor','$club_id','$inst_id','$Userid');";
                            ...
                            {     
                                do {
                                            ...
                                            if ($result = $conn->store_result()) 
                                            {
                                                ...
                                                $course_id = "sc_course_".$var."";
                                                $sqli = "UPDATE  school__live_course SET course_id = '$course_id' where sno= $var";         
                                                $conn->query($sqli);
                                                echo "success";
                                                $result->free();
