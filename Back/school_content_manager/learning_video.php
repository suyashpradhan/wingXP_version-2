...
if(isset($_GET['id'])){
    $req = 'select status from school__learning_video where video_id= "'.$_GET['id'].'" ';
    $resp = $conn->query($req);
    ...
    {
    $id = $_GET['id'];
    $vid_up = "SELECT title, description_line, duration, learning, vendor_id,topic_id,school_price, mrp_price, video_file,class_applicable_for,subscription_level,link
     from school__learning_video where video_id= '$id'";
...
<body >
    ...
    <div class="content">
        <h1 class="ribbon">Learning Video:</h1>
    ...
    <div class="modal fade" id="youtube" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <p style="color:red;"><?php if(isset($file) && $file!=''){echo 'Currently you have uploaded a video, it will be removed if you use a youtube video';}else{}?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php if(isset($file) && $file!=''){ echo 'var fileToUpload=1;';}else{echo 'var fileToUpload=$("#fileToUpload").val();';}?>
    var topic=$('#topic').val();
