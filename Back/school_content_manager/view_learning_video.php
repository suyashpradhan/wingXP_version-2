...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $vid_up = "SELECT title, description_line, duration,link, school__topic.topic_name,learning, vendor.vendor_id,class_applicable_for,mrp_price, school_price,
    video_file,vendor.vendor_name,vendor.vendor_icon,activities.icon from school__learning_video
    INNER JOIN vendor ON 
    school__learning_video.vendor_id =   vendor.vendor_id  INNER JOIN school__topic ON 
    school__learning_video.topic_id =   school__topic.topic_id  INNER JOIN activities ON
...
?> Mins</h1>
            ...
            <?php if(isset($video_file) and $video_file==''){echo '<iframe  src="';}?><?php  if(isset($video_file) and $video_file==''){ echo $link;}
             if(isset($video_file) and $video_file==''){echo '" frameborder="0"
                            ...
                            <?php if(isset($link) and $link==''){echo '<video id="video1" class="local-video" controls>
                                <source src="../assets/learning_video/';}?><?php  if(isset($link) and $link==''){echo $video_file;}?><?php
    ...
    <input type="hidden" name="id" id="id" value="<?php if(isset($id)){echo $id;}else{}?>">
           <input type="hidden" id="type" name="type" value="learning_video">
