...
if(isset($_GET['id'])){
    ...
    $vid_up = ($flag == 0 ? "SELECT name, author, duration, topic.topic_name,description, mrp_price,school_price,class_applicable_for,ebook_file,
    vendor.vendor_name,vendor.vendor_icon,activities.icon from ebook INNER JOIN vendor ON ebook.vendor_id =   vendor.vendor_id 
    INNER JOIN topic ON ebook.topic_id =   topic.topic_id INNER JOIN activities ON activities.page_name LIKE 'ebook.php' where
     book_id= '$id' and ebook.status='1'":
     'SELECT name, author, duration, school__topic.topic_name,description, mrp_price,school_price,class_applicable_for,ebook_file,
     vendor.vendor_name,vendor.vendor_icon,activities.icon from school__ebook INNER JOIN vendor ON school__ebook.vendor_id =   vendor.vendor_id 
     INNER JOIN school__topic ON school__ebook.topic_id =   school__topic.topic_id INNER JOIN activities ON activities.page_name LIKE "ebook.php" where
      book_id= "'.$id.'" and school__ebook.inst_id="'.$_SESSION['inst_id'].'"');
