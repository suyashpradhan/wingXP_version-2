...
if(isset($_GET['id'])){
    ...
    $vid_up = ($flag == 0 ? "SELECT description_line,description, duration, learning, topic.topic_name,
    vendor.vendor_name, mrp_price, school_price,class_applicable_for,vendor.vendor_icon,activities.icon,primary_image,secondary_image,course_icon
     from live_course  INNER JOIN vendor ON live_course.vendor_id =   vendor.vendor_id  INNER JOIN topic ON live_course.topic_id =   topic.topic_id 
    INNER JOIN activities ON activities.page_name LIKE 'course.php' where course_id= '$id' and live_course.status='1'":
    'SELECT description_line,description, duration, learning, school__topic.topic_name, vendor.vendor_name, mrp_price, school_price,class_applicable_for,
    vendor.vendor_icon,activities.icon,primary_image,secondary_image,course_icon from school__live_course  INNER JOIN vendor ON 
    school__live_course.vendor_id =   vendor.vendor_id  INNER JOIN school__topic ON school__live_course.topic_id
      =   school__topic.topic_id INNER JOIN activities ON activities.page_name LIKE "course.php" where course_id= "'.$id.'" and 
      school__live_course.inst_id="'.$_SESSION['inst_id'].'"');
