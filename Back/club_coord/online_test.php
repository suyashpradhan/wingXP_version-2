...
if(isset($_GET['id'])){
    ...
    $vid_up = ($flag == 0 ? "SELECT test_name, test_creator, duration, test_type, topic.topic_name,school_price,mrp_price,class_applicable_for,test_data,
    vendor.vendor_name,vendor.vendor_icon,activities.icon from online_test INNER JOIN vendor ON online_test.vendor_id =   vendor.vendor_id 
    INNER JOIN topic ON online_test.topic_id =   topic.topic_id  INNER JOIN activities ON activities.page_name LIKE 'online_test.php' where 
    test_id= '$id' and online_test.status='1'":
    'SELECT test_name, test_creator, duration, test_type, school__topic.topic_name,school_price,mrp_price,class_applicable_for,test_data,
    vendor.vendor_name,vendor.vendor_icon,activities.icon from school__online_test INNER JOIN vendor ON school__online_test.vendor_id =   vendor.vendor_id 
    INNER JOIN school__topic ON school__online_test.topic_id =   school__topic.topic_id  INNER JOIN activities ON activities.page_name LIKE "online_test.php" where 
    test_id= "'.$id.'" and school__online_test.inst_id="'.$_SESSION['inst_id'].'"');
