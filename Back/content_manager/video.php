...
if(isset($_GET['id'])){
    $req = 'select status from video where video_id= "'.$_GET['id'].'" ';
    $resp = $conn->query($req);
...
?>
    ...
    <div class="modal fade" id="youtube" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            ...
            <div class="left-right-center-div">
                ...
                <select name="tags[]" class="__select" multiple>  
                <?php 
                    $sql = "SELECT tag_id,tag_name FROM tags WHERE topic_id = '".$_SESSION['topic_id']."' and status='1'"; 
                    $result = $conn->query($sql);
                    $q='SELECT tag_id from tag_assign where type="video" and activity_id="'.$_GET['id'].'"';
        ...
        if(fileToUpload !== '' && link !== '')
    {
        ...
        if(duration == '' || editor1 == '' || editor2 == '' || vendor == '' || price == '')
                  {
