...
if(isset($_GET['id'])){
    $req = 'select status from school__video where video_id= "'.$_GET['id'].'" ';
    $resp = $conn->query($req);
    ...
    {
    $id = $_GET['id'];
    $vid_up = "SELECT title, description_line, duration, learning, vendor_id,topic_id,school_price, mrp_price, video_file,
    class_applicable_for,subscription_level,link from school__video where video_id= '$id'";
