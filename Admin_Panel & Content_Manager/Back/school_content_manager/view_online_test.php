...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $vid_up = "SELECT test_name, test_creator, duration, test_type, topic.topic_name,
    school_price,mrp_price,class_applicable_for,test_data,vendor.vendor_name,vendor.vendor_icon,activities.icon from school__online_test 
    INNER JOIN vendor ON 
    school__online_test.vendor_id =   vendor.vendor_id INNER JOIN topic ON 
    school__online_test.topic_id =   topic.topic_id  INNER JOIN activities ON
...
?> Mins</h1>
    ...
    <input type="hidden" name="id" id="id" value="<?php if(isset($id)){echo $id;}else{}?>">
           <input type="hidden" id="type" name="type" value="online_test">
