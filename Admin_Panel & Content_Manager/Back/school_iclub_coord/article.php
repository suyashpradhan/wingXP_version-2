...
if(isset($_GET['id'])){
    $req = 'select status from school__article where article_id= "'.$_GET['id'].'" ';
    $resp = $conn->query($req);
    ...
    {
    $id = $_GET['id'];
    $art_up = "SELECT name, description,  author, school_price,mrp_price,vendor_id,topic_id,class_applicable_for,subscription_level,link,icon,featured
     from school__article where article_id= '$id'";
...
<body style="background: linear-gradient(to left, #6190e8, #a7bfe8);">
    ...
    <input type="checkbox" class="option-input radio" name="featured" value="1"  id="" <?php if(isset($feat)){echo ($feat=='1') ? 'checked="checked"' : '';}else{}?> /> 
        <span class="text-head">Set as Featured Article</span> <div class="left-right-center-div">
                <div class="input-group">
    ...
    var fileToUpload= $('#fileToUpload').val();      
           if(fileToUpload == '' || title == '' || editor1 == '' || price == ''  || school_price == '' || vals == '' || topic == '' )
                  {
