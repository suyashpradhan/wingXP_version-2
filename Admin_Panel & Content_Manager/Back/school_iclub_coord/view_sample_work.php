...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $vid_up = "SELECT title,last_date,link,media_type, description_line,image, duration,topic.topic_name,
     learning, vendor.vendor_id,class_applicable_for,mrp_price, school_price,
     video_file,vendor.vendor_name,vendor.vendor_icon,activities.icon from school__sample_work
    INNER JOIN vendor ON 
    school__sample_work.vendor_id =   vendor.vendor_id INNER JOIN topic ON 
    school__sample_work.topic_id =   topic.topic_id INNER JOIN activities ON
...
<h1 class="desc-text" style="float:right;margin-right: 15px;">Last Date : <?php if(isset($l_date)){echo $l_date;}else{}?></h1>
        ...
        <div class="test-section">
            ...
            <?php if(isset($link) and $link!='' AND $media == 'link'){echo '<iframe  src="';}?><?php  
            if(isset($link) and $link!='' AND $media == 'link'){ echo $link;} if(isset($link) and $link!='' AND $media == 'link'){echo '" frameborder="0"
                            encrypted-media allowfullscreen allow="autoplay" ;></iframe>';}?>
                            <?php if(isset($video_file) and $video_file!='' AND $media == 'video'){echo '<video id="video1" class="local-video" controls>
                                <source src="../assets/sample_work/';}?><?php  if(isset($video_file) and $video_file!='' AND $media == 'video')
                                {echo $video_file;}?><?php  if(isset($video_file) and $video_file!='' AND $media == 'video'){echo '" width="400" type="video/mp4" controls>   
    ...
    <input type="hidden" name="id" id="id" value="<?php if(isset($id)){echo $id;}else{}?>">
           <input type="hidden" id="type" name="type" value="sample_work">
