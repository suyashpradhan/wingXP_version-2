...
if(isset($_GET['id'])){
    $req = 'select status from learning_video where learning_video_id= "'.$_GET['id'].'" ';
    $resp = $conn->query($req);
    ...
    {
    $id = $_GET['id'];
    $vid_up = "SELECT title, description_line, duration, learning, vendor_id,topic_id,school_price, mrp_price, video_file,class_applicable_for,subscription_level,link from learning_video where video_id= '$id'";
...
?>
        ...
        if(fileToUpload !== '' && link !== '')
    {
        ...
        if(course == '' || duration == '' || editor1 == '' || editor2 == '' || vendor == '' || price == '')
                  {
