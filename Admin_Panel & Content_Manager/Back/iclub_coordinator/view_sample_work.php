...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $vid_up = "SELECT sample_work.title,sample_work.last_date,topic.topic_name,sample_work.link,sample_work.media_type,
     sample_work.description_line,sample_work.image, sample_work.duration,sample_work.link,
...
<h1 class="desc-text" style="float:right;margin-right: 15px;">Last Date : <?php if(isset($l_date)){echo $l_date;}else{}?></h1>
        ...
        <div class="test-section">
            ...
            <?php if($video_file=='' AND $media == 'link'){echo '<iframe  src="';}?><?php  if($video_file=='' AND $media == 'link'){ echo $link;} if($video_file=='' AND $media == 'link'){echo '" frameborder="0"
                            ...
                            <?php if(isset($pdf) AND $media == 'pdf'){echo '<div id="ebookfile"></div>';}else{}?>          </div>
            </div>
        </div>
        <div class="div-gap">
            <div class="last-wrap">
