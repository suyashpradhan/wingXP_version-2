...
if(isset($_POST['freq'])){
    switch($_POST['freq']){
        case 'daily': $date='timestamp(current_date)';        
        break;
        case 'weekly':$date='DATE_ADD(CURDATE(), INTERVAL -1 WEEK)';
        break;
        case 'monthly':$date='DATE_ADD(CURDATE(), INTERVAL -1 MONTH)';
        break;
        case 'total': $date='2018-02-18';
        break;
        default: $date='2018-02-18';    
    }
...
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO"
    ...
    <button name="freq" value="daily" class="btn <?php if(isset($_POST['freq']) and $_POST['freq']=='daily'){echo 'btn-success';}else{echo 'btn-default';}?>">Daily</button>
    <button name="freq" value="weekly" class="btn <?php if(isset($_POST['freq']) and $_POST['freq']=='weekly'){echo 'btn-success';}else{echo 'btn-default';}?>">Weekly</button>
    <button name="freq" value="monthly" class="btn <?php if(isset($_POST['freq']) and $_POST['freq']=='monthly'){echo 'btn-success';}else{echo 'btn-default';}?>">Montly</button>
    <button name="freq" value="total" class="btn <?php if(isset($_POST['freq']) and $_POST['freq']=='total'){echo 'btn-success';}else if(empty($_POST)){echo 'btn-success';}else{echo 'btn-default';}?>">Total</button>
    </form>
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <!-- CHANGE 1 START-->
            <li class="nav-item">
                <a class="nav-link" id="incomplete" data-toggle="tab" href="#tab-four" role="tab" aria-controls="tab-four"
                    aria-selected="false">Incomplete Registration</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="complete" data-toggle="tab" href="#tab-five" role="tab" aria-controls="tab-five"
                    aria-selected="false">Complete Registration</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="call-later" data-toggle="tab" href="#tab-six" role="tab" aria-controls="calling-pending"
                    aria-selected="false">Calling Pending</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="call-done" data-toggle="tab" href="#tab-seven" role="tab" aria-controls="calling-done"
                    aria-selected="false">Calling Done</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="login" data-toggle="tab" href="#tab-eight" role="tab" aria-controls="login-detail"
                    aria-selected="false">Last Login</a>
            </li>
            <!-- CHANGE 1 END-->
        </ul>
   ...
   $t2=mysqli_query($db,"select * from school__web_reg where datetime > ".$date." group by phone");
   ...
   $t2=mysqli_query($db,"select * from demo_user where type='contact' and datetime > ".$date." group by phone");
   $rows=mysqli_num_rows($t2);
   while($r=mysqli_fetch_array($t2))
   {  $count++;
   ?>
   ...
   mysqli_data_seek($t2,$r);
   $t2=mysqli_query($db,"select * from demo_user where type='demo' and datetime > ".$date." group by phone");
   $rows=mysqli_num_rows($t2);
   while($r=mysqli_fetch_array($t2))
   {  $count++;
   ?>
   ...
   mysqli_data_seek($t2,$r);
   $t2=mysqli_query($db,"select * from school__enquiry where status!='1' and datetime > ".$date." group by phone");
   $rows=mysqli_num_rows($t2);
   while($r=mysqli_fetch_array($t2))
   {  $count++;
   ?>
   ...
   mysqli_data_seek($t2,$r);
   $t2=mysqli_query($db,"select * from institution where datetime > ".$date."");
   $rows=mysqli_num_rows($t2);
   while($r=mysqli_fetch_array($t2))
   {  $count++;
   ?>
            ...
            <div class="tab-pane fade" id="tab-six" role="tabpanel" aria-labelledby="calling-pending">
                <br>
                <div style="overflow-x:auto;overflow-y:auto">
                    <table class="table table-bordered">
                        <thead class="thead-dark">
   ...
   mysqli_data_seek($t2,$r);
   $t2=mysqli_query($db,"select * from institution join (select id,action,MAX(datetime) from (select * from sales_comments where id in (select DISTINCT(id) from sales_comments) and datetime > $date order by datetime DESC) t  group by id) p on p.id=institution.sno");
   $rows=mysqli_num_rows($t2);
   while($r=mysqli_fetch_array($t2))
   {  
       if($r['action']=='call_later'){
       $count++;
   ?>
            <tbody>
                <tr>
            ...
            <div class="tab-pane fade" id="tab-seven" role="tabpanel" aria-labelledby="calling-done">
                <br>
                <div style="overflow-x:auto;overflow-y:auto">
                    <table class="table table-bordered">
                        <thead class="thead-dark">
   ...
   ?>
            ...
            <div class="tab-pane fade" id="tab-eight" role="tabpanel" aria-labelledby="login-detail">
                <br>
                <div style="overflow-x:auto;overflow-y:auto">
                    <table class="table table-bordered">
                        <thead class="thead-dark">
   ...
   mysqli_data_seek($t2,$r);
   $t2=mysqli_query($db,"select * from institution join (select id,type,MAX(datetime) as last_login from (select * from school__log_detail where id in (select DISTINCT(id) from school__log_detail) and datetime > $date order by datetime DESC) t  group by id) p on p.id=institution.institute_id");
   $rows=mysqli_num_rows($t2);
   while($r=mysqli_fetch_array($t2))
   {  if($r['type']=='school'){
       $count++;
   ?>
            <tbody>
                <tr>
                    ...
                    <td>
                        <?php echo date('d/m/Y',strtotime($r['last_login'])); ?>
                    </td>
                    <td>
                        <?php echo date('h:i:s a',strtotime($r['last_login'])); ?>
                    </td>
                </tr>
            </tbody>
            <?php }} ?>
            ...
            <input type='hidden' id='modaltype' name='type'>
                ...
                <div class="btn_wraps">
                                ...
                                <button class="btn btn-success mt-1 mb-1"  data-target="#call-now" data-toggle="modal" style="width: 100px">Call
                                    Now</button>
                                <button class="btn btn-danger mt-1 mb-1" data-target="#call-later-modal" data-toggle="modal" style="width: 100px">Call
                                    Later</button>
                            </div>
        </div>
    </div>
  ...
  <div id="call-later-modal" class="modal fade" role="dialog">
  <div class="modal-dialog">
