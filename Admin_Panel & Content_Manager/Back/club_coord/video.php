...
if(isset($_GET['id'])){
    ...
    $vid_up = ($flag == 0 ? "SELECT title, description_line, duration, link, topic.topic_name,learning, class_applicable_for,mrp_price, school_price,
    video_file,vendor.vendor_name,vendor.vendor_icon,activities.icon from video INNER JOIN vendor ON video.vendor_id =   vendor.vendor_id INNER JOIN 
    topic ON video.topic_id = topic.topic_id  INNER JOIN activities ON activities.page_name LIKE 'video.php' where video_id= '$id' and video.status='1'":
     'SELECT title, description_line, duration, link, school__topic.topic_name,learning, class_applicable_for,mrp_price, school_price,
     video_file,vendor.vendor_name,vendor.vendor_icon,activities.icon from school__video INNER JOIN vendor ON school__video.vendor_id =   vendor.vendor_id INNER JOIN 
     school__topic ON school__video.topic_id = school__topic.topic_id  INNER JOIN activities ON activities.page_name LIKE "video.php" where video_id= "'.$id.'"
      and school__video.inst_id="'.$_SESSION['inst_id'].'"');
...
?> Mins</h1>
    ...
    <form id="fileUploadForm" action="dep.php" method="POST">
        <div class="div-gap">
            <h1 class="desc-head">Activity Is Applicable For Class</h1>
        ...
        <div class="test-section">
            ...
            <?php if($video_file==''){echo '<iframe  src="';}?><?php  if($video_file==''){ echo $link;} if($video_file==''){echo '" frameborder="0"
                            encrypted-media allowfullscreen allow="autoplay" ;></iframe>';}?>
                            <?php if($link==''){echo '<video id="video1" class="local-video" controls>
                                <source src="../assets/video/';}?><?php  if($link==''){echo $video_file;}?><?php  if($link==''){echo '" width="400" type="video/mp4" controls>   
                            </video>';}?> </div>
        </div>
        <div class="div-gap">
            <div class="last-wrap">
