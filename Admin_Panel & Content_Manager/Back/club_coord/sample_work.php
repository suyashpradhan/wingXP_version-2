...
if(isset($_GET['id'])){
    ...
    $vid_up = ($flag == 0 ? "SELECT title,last_date,topic.topic_name,link,media_type, description_line,image,pdf, duration,learning, class_applicable_for,
    mrp_price, school_price,video_file,vendor.vendor_name,vendor.vendor_icon,activities.icon from sample_work INNER JOIN vendor ON 
    sample_work.vendor_id =   vendor.vendor_id  INNER JOIN topic ON sample_work.topic_id =   topic.topic_id INNER JOIN activities ON
     activities.page_name LIKE 'sample_work.php' where sample_work_id= '$id' and sample_work.status='1'":
     'SELECT title,last_date,school__topic.topic_name,link,media_type, description_line,image,pdf, duration,learning, class_applicable_for,
     mrp_price, school_price,video_file,vendor.vendor_name,vendor.vendor_icon,activities.icon from school__sample_work INNER JOIN vendor ON 
     school__sample_work.vendor_id =   vendor.vendor_id  INNER JOIN school__topic ON school__sample_work.topic_id =   school__topic.topic_id
      INNER JOIN activities ON activities.page_name LIKE "sample_work.php" where sample_work_id= "'.$id.'" and school__sample_work.inst_id="'.$_SESSION['inst_id'].'"');
...
<h1 class="desc-text" style="float:right;margin-right: 15px;">Last Date : <?php if(isset($l_date)){echo $l_date;}else{}?></h1>
        ...
        <div class="test-section">
            ...
            <?php if($media == 'link'){echo '<iframe  src="';}?><?php  if($media == 'link'){ echo $link;} if( $media == 'link'){echo '" frameborder="0"
                            encrypted-media allowfullscreen allow="autoplay" ;></iframe>';}?>
                            <?php if($media == 'video'){echo '<video id="video1" class="local-video" controls>
                                <source src="../assets/sample_work/';}?><?php  if( $media == 'video'){echo $video_file;}?><?php  if($media == 'video'){echo '" width="400" type="video/mp4" controls>   
                            </video>';}?>  <?php if(isset($image) AND $media == 'image'){echo '<img src=../assets/sample_work/'.$image.'>';}else{}?>  
                            <?php if(isset($pdf) AND $media == 'pdf'){echo '<div id="ebookfile"></div>';}else{}?>  
    ...
    <input type="hidden" name="id" value="<?php if(isset($id)){echo $id;}else{}?>">
        ...
        <script>PDFObject.embed("../assets/sample_work/<?php if(isset($pdf)){echo $pdf;}else{}?>", "#ebookfile");</script>
